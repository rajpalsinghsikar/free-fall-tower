...
function rightMeterChange(rvalue){
    var rightMeter = new Gauge("canvasRight", {'mode':'needle', 'range': {'min':0, 'max':500 } });
    rightMeter.draw(rvalue);
    }

function leftMeterChange(lvalue){
    var leftMeter = new Gauge("canvasLeft", {'mode':'needle', 'range': {'min':0, 'max':500} });
    leftMeter.draw(lvalue );
    }
...
function __needle__( g ) {
	...
	//draw scale
	for(var n=0; n <= g.max-g.min; n+=50) {
...
Gauge.prototype.draw = function(val) {
	...
	this.step = this.diff/32;
	
	this.animation = setInterval(animate, this.time/32 );
};
